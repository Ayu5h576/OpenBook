  ...
  return (
    <div className="space-y-8 pb-12">
        ...
        {isEditing ? (
          <div className="space-y-4">
            ...
            <input
              ...
              className="w-full bg-[var(--bg-ivory)] border border-[var(--border-light)] rounded-2xl px-4 py-3 text-2xl font-bold text-[var(--ink)] focus:outline-none focus:border-[var(--ink)]"
            />
            <textarea
              value={editDesc}
              ...
              rows={2}
              className="w-full bg-[var(--bg-ivory)] border border-[var(--border-light)] rounded-2xl px-4 py-3 text-sm text-[var(--ink)] focus:outline-none focus:border-[var(--ink)] resize-none"
            />
          ...
          <div className="flex items-start justify-between gap-6">
            <div>
              <h1 className="font-serif-title text-4xl font-bold text-[var(--ink)] mb-2">
                {collection.name}
              </h1>
              {collection.description && (
                <p className="text-sm text-[var(--muted)] leading-relaxed mb-4">{collection.description}</p>
              )}
      ...
      {(collection.books?.length ?? 0) === 0 ? (
        <div className="bg-[var(--white)] border border-[var(--border-light)] rounded-3xl p-12 text-center">
          <BookOpen className="w-10 h-10 text-[var(--border-light)] mx-auto mb-3" />
          <p className="text-sm text-[var(--muted)]">No books in this collection yet.</p>
          <p className="text-xs text-[#A0A0A0] mt-1">Add books from the Explore section or book detail pages.</p>
        </div>
      ) : filteredBooks.length === 0 ? (
        <div className="bg-[var(--white)] border border-[var(--border-light)] rounded-3xl p-12 text-center">
          <p className="text-sm text-[var(--muted)]">No books match "{searchQuery}"</p>
        </div>
      ) : (
